...
namespace smht
{
    ...
    {
        #if ON_WINDOWS
        return _InterlockedCompareExchange8(ptr, newval, expected);
        #else
        return __sync_val_compare_and_swap(ptr, expected, newval);
        #endif
    }
...
#pragma warning(error : 4302)
    ...
    ShMemHolder::ShMemHolder(const char* name, uint32_t size)
        : Size(size)
#if ON_WINDOWS
        , Handle(NULL)
#else
    ...
    uint32_t ShMemHolder::GetSize()
    {
        return Size;
    }

    void* ShMemHolder::GetAddress()
    {
